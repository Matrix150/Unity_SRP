...
float3 SampleLightMap(float2 lightMapUV)
{
    ...
    #else
        true,
    #endif      // return a ture/false
        float4(LIGHTMAP_HDR_MULTIPLIER, LIGHTMAP_HDR_EXPONENT, 0.0, 1.0));
#else
    return 0.0;
#endif
}
...
float3 SampleEnvironment(Surface surfaceWS, BRDF brdf)
{
    ...
    float4 enivronment = SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, uvw, mip);
    return DecodeHDREnvironment(enivronment, unity_SpecCube0_HDR);
}
