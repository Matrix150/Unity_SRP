...
{
    float m, n, e, s, w, ne, se, sw, nw; // middle, north, east, south, west (stands for direction)
...
float GetSubpixelBlendFactor(LumaNeighborhood luma)
{
    ...
    filter += luma.ne + luma.nw + luma.se + luma.sw;        // Different weight
    filter *= 1.0 / 12.0;      // low-pass filter
    filter = abs(filter - luma.m);      // high-pass filter
    filter = saturate(filter / luma.range);     // normalize filter
    filter = smoothstep(0, 1, filter);      // smooth filter
